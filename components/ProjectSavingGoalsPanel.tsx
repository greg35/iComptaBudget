...
import { ProjectSavingGoal, SavingGoalSuggestion, ProjectMonthlyGoalPerformance } from '../types/budget';
...
export function ProjectSavingGoalsPanel({ projectId }: Props) {
  ...
  async function loadAll() {
    ...
    try {
      const [histRes, suggRes, perfRes] = await Promise.all([
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <InfoBlock icon={<TrendingDown className="h-4 w-4" />} label="Reste à épargner" value={suggestion ? fmt(suggestion.remainingBudget)+'€' : '—'} />
        </div>
        <div className="pt-2">
